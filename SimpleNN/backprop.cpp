...
double *alloc_1d_dbl(int n)
{
  ...
  new_ = (double *) malloc ((unsigned) (n * sizeof (double)));
  if (new_ == NULL) {
...
double **alloc_2d_dbl(int m, int n)
{
  ...
  new_ = (double **) malloc ((unsigned) (m * sizeof (double *)));
  if (new_ == NULL) {
...
//  FILE *fp;
//
//  if (!(fp = fopen(filename, "w+"))/*(fd = creat(filename, 0644)) == -1*/) {
...
//
//  fwrite((char *) &n1, sizeof(int),1 , fp);
//  fwrite((char *) &n2, sizeof(int),1 , fp);
//  fwrite((char *) &n3, sizeof(int),1 , fp);
//
//  memcnt = 0;
//  w = net->input_weights;
...
//  fwrite(mem, sizeof(double), (n1+1)*(n2+1), fp);
//  free(mem);
//
//  memcnt = 0;
...
//  fwrite(mem, sizeof(double), (n2+1)*(n3+1), fp);
//
//  free(mem);
//
//  fclose(fp/*fd*/);
